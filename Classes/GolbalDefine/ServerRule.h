...
#define SR_NO_WIN_LOSE				0x01000000							//不列入输赢
#define SR_SAME_IP					0x02000000							//禁止同ip同桌
#define SR_IS_TRAIN_ROOM         	0x04000000							//是否是体验场
//////////////////////////////////////////////////////////////////////////////////
...
public:
	...
	//不列入输赢
	static bool IsNoWinLose(DWORD dwServerRule) { return (dwServerRule&SR_NO_WIN_LOSE)!=0; }
	//禁止同ip同桌
	static bool IsSameIP(DWORD dwServerRule) { return (dwServerRule&SR_SAME_IP)!=0; }
    
    //是否是体验场
    static bool IsTrainRoom(DWORD dwServerRule) { return (dwServerRule&SR_IS_TRAIN_ROOM)!=0; }
...
public:
	...
	//不列入输赢
	static VOID SetNoWinLose(DWORD & dwServerRule, bool bEnable) { (bEnable==true)?dwServerRule|=SR_NO_WIN_LOSE:dwServerRule&=~SR_NO_WIN_LOSE; }
	//禁止同ip同桌
	static VOID SetSameIP(DWORD & dwServerRule, bool bEnable) { (bEnable==true)?dwServerRule|=SR_SAME_IP:dwServerRule&=~SR_SAME_IP; }
    //是否是体验场
    static VOID SetTrainRoom(DWORD & dwServerRule, bool bEnable) { (bEnable==true)?dwServerRule|=SR_IS_TRAIN_ROOM:dwServerRule&=~SR_IS_TRAIN_ROOM; }
